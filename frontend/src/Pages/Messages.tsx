...
const Messages: React.FC = () => {
    ...
    // Fetch logged-in user and chat user data
    useEffect(() => {
        const fetchUserData = async () => {
            if (isAuthenticated && user?.email) {
                setLoading(true);
                try {
                    const loggedUserId = await checkUser(user.email);
                    setLoggedInUserId(loggedUserId);

                    if (attendeeId) {
                        await fetchChatUser(attendeeId);
                        await fetchMessages(loggedUserId, attendeeId);
                    }
                } catch (err) {
        ...
        fetchUserData();
    }, [isAuthenticated, user, attendeeId]);

    const checkUser = async (email: string): Promise<number> => {
        const response = await fetch(`${API_BASE_URL}/users/by-email/${email}`, {
            method: "GET",
            headers: { "Content-Type": "application/json" },
        });
    ...
    const createUser = async (): Promise<number> => {
    ...
    const fetchMessages = async (senderId: number, receiverId: string) => {
        const response = await fetch(
            `${API_BASE_URL}/Messages/messages/${senderId}/${receiverId}`
        );
        if (response.ok) {
            const data = await response.json();
            setMessages(data);
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-100">
                ...
                {loading ? (
                    ...
                    messages.map((msg, index) => (
                        <div
                            ...
                            className={`flex ${msg.sender_id === loggedInUserId ? "justify-end" : "justify-start"}`}
                        >
                            <div className="p-2 rounded-lg shadow-md bg-gray-200">
                                {msg.content}
                                {msg.sender_id === loggedInUserId && (
                                    <button
                                        onClick={() => handleDeleteMessage(msg.id)}
            ...
            <div className="sticky bottom-0 flex items-center gap-2 p-4 border-t bg-white shadow-md">
                ...
                <button
                    onClick={handleSendMessage}
                    className="bg-green-500 hover:bg-green-600 text-white font-bold px-4 py-2 rounded-full shadow-md"
                >
                    Send
                </button>
            </div>
